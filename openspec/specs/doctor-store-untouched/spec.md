# Doctor Store (Untouched Boundary) Spec
...
This is a sentinel spec whose sole purpose is to call out the boundary
between the new service-only Doctores page and the legacy NgRx slice still
consumed by `add-patient-v2` and the deprecated `doctor-form.component.ts`.
...
This spec explicitly declares that boundary so future reviewers do not
